...
function initApp() {
    // Initialize all jQuery element variables now that the HTML is loaded.
    initElements();

    // Initialize all other application modules.
    initSounds();
    initLogBox();
    ...
    // ===================================================================================
    //  NEWLY ADDED EVENT LISTENERS FOR SETTINGS
    // ===================================================================================
    ...
    themeSelector.on('change', function() {
        ...
        buildBoard(game.fen()); // Rebuild board to apply new theme
        Logger.info(`Board theme changed to: ${themeName}`);
    });
    
    pieceThemeSelector.on('change', function() {
        const themeName = $(this).val();
        localStorage.setItem('chessPieceTheme', themeName);
        buildBoard(game.fen()); // Rebuild board to apply new pieces
        updateCapturedPieces(); // Update captured pieces as well
        Logger.info(`Piece theme changed to: ${themeName}`);
    });
    ...
    timeControlSelector.on('change', function() {
        ...
        Logger.info(`Default time control saved: ${TIME_CONTROLS[selectedKey].label}`);
    });
    ...
    Promise.all([
        ...
        // --- Populate Settings Dropdowns On Startup ---
...
$(document).ready(function() {
    // initTemplates() is a synchronous function that injects HTML into the page.
    initTemplates();
    
    // Because the HTML is now guaranteed to be present, we can immediately initialize
    // the main application logic without waiting for a promise.
    initApp();
});