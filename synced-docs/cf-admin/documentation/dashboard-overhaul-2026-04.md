...
4. **Row 2 (2-column):** Edge Analytics 24h chart (responsive) | Supabase Platform (GoTrue Auth + PostgreSQL metrics + Active App Users)
...
**Note:** The former "Business Engine" ticket card was removed. Its only real stat ("Active Application Users") was moved into the Supabase Auth widget where it is more contextually relevant alongside Auth and MAU data.
...
5. **Supabase PostgreSQL Metrics** — Fetches Prometheus metrics from Supabase (database size, CPU load, active connections, RAM). Also hits the Management API for backup database size data.

6. **Supabase Auth (GoTrue)** — Fetches total registered users and monthly active users using the GoTrue Admin API and Management API.
...
**Sections:**
- **Authentication** — Total Users, Monthly Active Users, Active App Users (24h) from SSR audit log query
- **PostgreSQL Edge Cluster** — 3-column metric grid: CPU Load, Active Connections, RAM Utilization percentage
- DB size progress bar against 500 MB plan limit
- Micro-bar color thresholds: green < 60%, amber 60–80%, red ≥ 80%
...
| Supabase DB | cpuLoad ≤ 2 | cpuLoad > 2 | — | unconfigured |
...
- [ ] SupabaseAuthWidget shows real total user count
...
2. **Supabase MAU on free tier** — The MAU field is typically absent from the Supabase Management API usage endpoint on free plans. MAU will show as 0 unless Supabase exposes this.
...
5. **Supabase Prometheus endpoint** — The privileged metrics endpoint is a non-public API. It has worked reliably but Supabase could change or restrict it without notice.
