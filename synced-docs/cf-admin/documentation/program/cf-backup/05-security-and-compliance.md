...
| T23 | **Privilege escalation through the access policy** | `access.manage` is an Owner/Vendor floor; the secret class, `keys.status` and `runs.download` cannot be granted below Owner; deny beats allow; unknown capabilities are denied; compare-and-swap writes; every change audited, with a notice to the alert recipients (doc 13 §2, §3) |
...
| T25 | **The dump credential can read backup keys** (`pg_read_all_data` reads every schema, including Vault) | Every run's `doctor` proves the reader is refused on `vault.decrypted_secrets` and stops the Postgres stage otherwise. **Taken (2026-09-23):** the proof failed under `pg_read_all_data`, so the reader now holds per-schema grants and is refused (doc 12 §5) |
...
| 4 | The Vault login (`backup_keyholder`), as built in the Hyperdrive config bound as `VAULT_DB` (it was the Worker secret `SUPABASE_KEYS_URL` until 2026-09-24) | cf-backup Worker (binding) | `EXECUTE` on the three backup-key functions only |

Not secrets: `CLOUDFLARE_ACCOUNT_ID` and `BACKUP_AGE_RECIPIENT` (GitHub variables), the
GitHub App ID (a console setting, Settings → GitHub). The backup **private** keys live only in Supabase Vault,
