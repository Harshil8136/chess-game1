...
- **Alerts:** the job enqueues each of the tick's `alerts[]` on cf-admin's own `EMAIL_QUEUE` (`purpose: 'custom_email'`, `projectSource: 'cf-admin'`), then calls `POST /internal/alerts/ack {ids}` with the ids that sent. An alert not acked is offered again next tick; the email consumer dedupes repeats, so a warning goes out at least once, not twice. Since 2026-09-24 each alert may carry a `sender` (cf-backup's `alerts.senderEmail`/`senderName`); the job checks it again (an address on madagascarhotelags.com, a name without quotes, brackets or control characters) and puts it in the message's `data.from`, which the consumer uses for `custom_email`; without one, or when it is refused, the consumer sends from its `SENDER_EMAIL`. The address must be a verified sender in Brevo, or the provider rejects the email.
