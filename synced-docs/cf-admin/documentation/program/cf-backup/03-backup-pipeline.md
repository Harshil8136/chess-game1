...
| Supabase Postgres 17 | 17 MB | **`supabase db dump`, not raw `pg_dump`** (factor B1): `roles.sql` (`--role-only`), `schema.sql`, `data.sql` (`--data-only --use-copy`), plus the `supabase_migrations` schema and data pair. Includes RLS policies, functions, triggers and pg_cron job definitions. **As built: not `auth.*` data** (the sign-in accounts): the backup role cannot read Supabase's `auth` schema, so every run records `auth.users` as skipped, with a warning (doc 12 §5) | **Confirmed** from Supabase's own backup/restore guides: raw `pg_dump` pulls in Supabase internals and fails on restore. Customisations to `auth`/`storage` are captured by `supabase db diff --schema auth,storage` if any exist (factor B5) |
...
| S-3 | Can a dedicated `backup_reader` role get `pg_read_all_data` on Supabase, including `auth` and `cron`? | Dump with the `postgres` role via the pooler; keep the URL only in GitHub secrets. **Answered 2026-09-23/24:** it could, but `pg_read_all_data` also read `vault.decrypted_secrets`, so it was revoked for per-schema grants; the `auth` grant does not take (Supabase owns the schema), so `auth.*` is skipped in every run (doc 12 §5) |
