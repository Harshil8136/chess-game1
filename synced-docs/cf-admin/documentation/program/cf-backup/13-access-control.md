...
<!-- Built and deployed 2026-09-23/24; the "As built" notes record where the build differs from the plan. It names cf-backup routes, settings keys and screens, which live in cf-backup's repository, not cf-admin's. -->
...
goes to **the alert recipients** set in the console (Settings → Alerts), not to named
people: there are none by default, and until someone is added every notice waits, which
the Alerts screen says. Settings → Notifications sends notices at once or in the daily
digest; they cannot be switched off email, because a notice is how the other key holder
learns of a download or a reveal. Each email's delivery is tracked: cf-admin's backup-tick
job writes its `email_audit_logs` row (status `queued`, like the Email Portal's own sends)
and puts it on `EMAIL_QUEUE`; the email consumer updates that row as the provider answers;
on a later tick cf-admin reads the row and reports the delivery back
(`POST /internal/alerts/delivery`), and the Alerts screen shows delivered, bounced or
failed. "Fresh sign-in" comes from the gateway's `signedInAt` (doc 02 §4).

**`files.download` and GitHub's log archive** (as built, 2026-09-24): `files.download` is
grantable per person, and it reaches every non-data evidence file, including the stored
`postrun/github-logs.zip`. Doc 11 §4.5 describes that archive as Admin and above; a grant
of `files.download` to someone below Admin goes beyond that, so grant it knowingly. Backup
data stays Owner and Vendor only (the `runs.download` floor).
...
7. **Effect is prompt.** The policy row is read at most every 30 s per isolate, so a change applies within half a minute. *(As built: the 30 s settings cache exists, with write-through on save.)*
...
|---|---|---|
| Alerts | Recipients; which events email (failed, warning, stale, allowance ≥ 80%); quiet hours; the sender address and name | `failed` and `stale` cannot be switched off. **As built:** per-kind routing moved to a Notifications group (at once, the daily digest, or dashboard only; failed, lost, stale, key and notice alerts can never be dashboard only) |
...
| Run guards | Run-now cooldown (hours); business-hours warning window; **`manualRunsPerPersonPerDay`** (as built, Ruling R-4) | cooldown 1–24 h; per-person daily limit 1–24, default 6. **As built:** the business-hours window (UTC) is a warning in the Run now dialog; it never refuses a run |
| Live view | Refresh while active, refresh while idle, pause after inactivity (doc 14 §3) | **as built:** active refresh 5–60 s (default 5), idle 5–300 s (default 30), pause 5–240 min (default 15) |
...
`schedule.toggle`: which weekdays and time (UTC) each scope runs, the grace hours a busy
slot may wait, and whether the first full of the month is a real-path drill. cf-admin's
five-minute `backup-tick` job dispatches each due slot. The workflow file holds only the
D-5 fallback `schedule:` line (a full backup when none succeeded in 8 days), which the
console cannot change.
