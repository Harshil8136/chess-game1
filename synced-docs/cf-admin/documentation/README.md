...
| [`program/chunks/2026-09-10-07-cron-consolidation-and-job-observability.md`](program/chunks/2026-09-10-07-cron-consolidation-and-job-observability.md) | Chunk 7 record: the inert `*/15` trigger deleted and its jobs folded into `*/5` (account crons 4/5 → 3/5), cf-chatbot's every-minute tick slowed to `*/5`, the ten placeholder job budgets replaced with measured values, timeouts on the two untimed cron-path fetches, and the observability contract — no console-only catch on a job path, enforced by new ratchet metric A19 | ai, technical, owner | draft |
