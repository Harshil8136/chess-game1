...
| [`program/cf-backend/README.md`](program/cf-backend/README.md) | cf-backend plan of record (2026-09-21, for owner review): the operations-plane Worker, verdict on the original backup idea, owner decisions OD-1…OD-10, free-tier budget | owner, ai, technical, operator | draft |
| [`program/cf-backend/01-architecture.md`](program/cf-backend/01-architecture.md) | cf-backend architecture: private RPC entrypoint + narrow public `storage.*` surface, bindings, what moves out of cf-admin, failure isolation | ai, technical, owner | draft |
| [`program/cf-backend/02-admin-integration-contract.md`](program/cf-backend/02-admin-integration-contract.md) | How cf-admin renders cf-backend from a versioned manifest (server-driven UI, closed vocabulary), RPC envelope, fail-closed permissions, the `external` cron job kind, never-break rules | ai, technical, owner | draft |
| [`program/cf-backend/03-backup-pipeline.md`](program/cf-backend/03-backup-pipeline.md) | Weekly full + daily Supabase backup on GitHub Actions into R2: scope, R2 layout v1, public-key encryption, deterministic verification, manifest ledger (no new tables), manual retention with bucket lock | ai, technical, owner, operator | draft |
| [`program/cf-backend/04-staff-storage-move.md`](program/cf-backend/04-staff-storage-move.md) | Moving Staff Managed Storage to cf-backend and `storage.madagascarhotelags.com` in five reversible stages without breaking a live link | ai, technical, owner | draft |
| [`program/cf-backend/05-security-and-compliance.md`](program/cf-backend/05-security-and-compliance.md) | cf-backend threat model, secrets inventory by location, GitHub Free constraints, key custody, RoPA/DR/erasure updates | owner, ai, technical | draft |
| [`program/cf-backend/06-roadmap.md`](program/cf-backend/06-roadmap.md) | cf-backend phases P0–P5 with stages, exit criteria, owner steps and rollback | owner, ai, technical, operator | draft |
| [`program/cf-backend/07-factor-register.md`](program/cf-backend/07-factor-register.md) | cf-backend deep review (2026-09-21): 40 factors across platform limits, backup correctness, integration, security and recovery time — each confirmed / to verify / design, with where it changed the plan | owner, ai, technical | draft |
| [`program/cf-backend/08-existing-backup-workflow.md`](program/cf-backend/08-existing-backup-workflow.md) | The existing cf-admin `backups.yml` as it is (2026-09-21): every job and step, secrets state, run history (never produced a backup), gaps G1–G14, the bridge to a first real backup, and the mapping into cf-backend | owner, ai, technical, operator | draft |
| [`program/cf-backend/09-key-management.md`](program/cf-backend/09-key-management.md) | cf-backend backup key management: two-provider custody (backups at Cloudflare, key in Supabase Vault), Owner/Vendor-only one-click rotation, weekly automatic key check, recovery kit, what-if table, compliance text to add once built | owner, ai, technical, operator | draft |
| [`program/cf-backend/10-free-tier-feasibility.md`](program/cf-backend/10-free-tier-feasibility.md) | cf-backend free-tier feasibility: per-service verdict, the GitHub Actions budget against the under-1,000-minute target, R2 storage for years of backups, key rotation and re-keying at $0, Phase 0 checks, tripwires | owner, ai, technical, operator | draft |
