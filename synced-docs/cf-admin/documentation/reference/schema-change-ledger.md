...
36 files today (to `0058`), 36 rows — re-count with `ls migrations/*.sql | wc -l` rather than trusting this. Supabase (`supabase/migrations/`, 11 files) and
...
| `migrations/0057_backup_runs.sql` | 2026-09-23 | claude (CB-2, cf-backup integration) — **pending**, applies with `npx wrangler d1 migrations apply madagascar-db --local` then `npm run release` (owner) | Creates `backup_runs`, cf-backup's one table (one row per backup, restore-drill or prune attempt; a partial unique index is the single-active-run lock; `schedule_slot` UNIQUE makes scheduled dispatch idempotent), accepted by owner decision 2026-09-23 — the RULE #0.9 reuse proof is the file's own header. Also registers `/dashboard/backup` in `admin_pages` (`super_admin`, icon `database`, `system`, sort 89). cf-admin's own code never reads the table; cf-backup and its runner do. Additive only — no `ALTER`, no `-- contract:` marker needed. `database/schema.snapshot.sql` is regenerated after the release, not before |
| `migrations/0058_backup_runs_live_index.sql` | 2026-09-24 | claude (cf-backup program P2b, live view) — **pending**, applies with `npx wrangler d1 migrations apply madagascar-db --local` then `npm run release` (owner) | Adds one **partial** index on `backup_runs`: `backup_runs_actions_finished (finished_at) WHERE lane = 'actions' AND finished_at IS NOT NULL`, so the cf-backup console's live view finds runs that finished in the last minute with one range search. Without it the only fitting index (`backup_runs_kind_scope`) answers through 28 IN-list probes: measured live 2026-09-24, `rows_read` 24 per poll at 2 rows. Local `EXPLAIN QUERY PLAN` with the index: `SEARCH ... USING INDEX backup_runs_actions_finished (finished_at>?)` at 0, 2 and 400 rows. Additive; no `ALTER`, no `-- contract:` marker. The query answers correctly before it is applied (by reading the table), so the deploy order of the two repos does not matter. |
