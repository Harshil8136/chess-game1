...
- **`_unconfigured` flag pattern** on every provider â€” UI shows `â€”` / "Setup Required" instead of zeros when token is missing
- **Dismissible setup banner** when API token is not configured
- **ResizeObserver** drives chart width responsively
- **2-column bento grid** replaces old fixed layout â€” collapses to 1-column on narrow viewports

---

## Current Dashboard Layout (Top â†’ Bottom)

The dashboard is composed of the following visual rows, from top to bottom:

1. **Setup Banner** â€” Amber, dismissible â€” only visible if analytics token is missing
2. **Dashboard Header** â€” "Overview" title + Health Orb Dropdown + Refresh Button
...
5. **Storage & Queues** â€” Full-width (R2 + D1 + Email Queue)
6. **Quick Actions** â€” Full-width action buttons
7. **Audit Log Feed** â€” Full-width, 8 recent entries
...
1. **Zone HTTP Metrics** â€” Fetches zone-level HTTP analytics from Cloudflare GraphQL (requests, cached, bandwidth, threats, time-series for the chart). Also queries D1 analytics for read/write stats.

2. **Worker Invocation Analytics** â€” Fetches account-level Worker invocation data from Cloudflare GraphQL. CPU/wall times are converted from microseconds to milliseconds. Both worker scripts always appear in output, even with 0 invocations.

3. **R2 Bucket Usage** â€” Fetches object count and uploaded bytes from the R2 REST API.

4. **Queue Info** â€” Fetches queue metadata (name, consumer count) from the Cloudflare REST API.

5. **Supabase PostgreSQL Metrics** â€” Fetches Prometheus metrics from Supabase (18+ metrics including tuple activity, cache hit ratio, deadlocks, WAL, and disk utilization).
6. **Supabase Auth (GoTrue)** â€” Fetches total registered users, MAU (derived directly from `last_sign_in_at`), recent signups (7d), and auth provider breakdown using the GoTrue Admin API.

7. **Sentry Error Count** â€” Fetches error event counts from the Sentry API.

8. **Resend Email Stats** â€” Fetches email delivery stats (sent vs bounced) from the Resend API.

---

## Bug History â€” GraphQL Filter Field Names
...
**D1 analytics bug:** Originally missing datetime filter entirely â€” could return all-time data or error. Fixed to include proper datetime scope.
...
### Workers Widget
- When unconfigured â†’ renders a "Workers Analytics Unavailable" setup state
- When loading â†’ skeleton spans inside each worker card
...
1. **R2 Images** â€” object count + formatted bytes vs 10 GB limit
2. **D1 Database** â€” read queries, write queries, rows read, rows written
3. **Email Queue** â€” green status dot, queue name, consumers count

Renders full-width using the teal-tinted bento card variant.

### Supabase Cluster Widget (formerly SupabaseAuthWidget)
**Tabbed Layout:**
- **Tab 1: PostgreSQL Performance** â€” Comprehensive performance overview including a 4-column load/RAM/connections grid, prominent Buffer Cache Hit Ratio bar, transaction counters (commits, rollbacks, deadlocks), tuple activity, and disk/WAL utilization bars. Progress bar against 500 MB plan limit.
- **Tab 2: GoTrue Auth** â€” Total Registered, Active Now (24h) with pulse dot, MAU (30d), and Recent Signups (7d). Includes an Auth Provider Breakdown horizontal bar chart showing Google/Email/Phone/etc identities.
...
**KV entries removed** â€” Cloudflare does not expose KV operation counts via any API.

When unconfigured â†’ progress bar shows striped pattern, value area shows "Token required".
...
|---------|-------------|----------|--------|---------|
| API Gateway | error rate â‰¤ 1% | 1â€“5% | > 5% | workers unconfigured |
| cf-astro Worker | error rate â‰¤ 1% | 1â€“5% | > 5% | workers unconfigured |
| Database (D1) | configured | â€” | â€” | unconfigured |
| Session KV | always operational | â€” | â€” | never unknown |
| R2 Storage | configured | â€” | â€” | unconfigured |
| Email Queue | queue present | â€” | â€” | queue absent |
| Supabase DB | cpuLoad â‰¤ 2 AND cacheHitRatio â‰¥ 0.90 AND ramUsedPct â‰¤ 0.90 | cpuLoad > 2 OR cacheHitRatio < 0.90 OR ramUsedPct > 0.90 | deadlocks > 0 OR cpuLoad > 4 | unconfigured |
| GoTrue Auth | configured | â€” | â€” | unconfigured |
| Resend Email | configured | â€” | â€” | unconfigured |
| Sentry | configured | â€” | â€” | unconfigured |

**Overall label logic:**
- Any outage â†’ "Partial Outage"
- Any degraded â†’ "Degraded"
- All known operational â†’ "All Systems Operational"
- No known statuses â†’ "Awaiting Data"
...
### Layout Classes
- **2-column bento row** â€” collapses to 1-column at â‰¤1100px viewport
- **Workers split** â€” 2-column side-by-side cards (collapses to 1-column at â‰¤640px)
- **Quota grid** â€” 3-column grid with cubic-bezier animated progress bars
- **Gradient variants** â€” blue-tinted (Workers), teal-tinted (Storage), amber/green-tinted (Quota), green-tinted (Supabase)
...
After every Cloudflare GraphQL response, the code checks for errors before reading data. Cloudflare returns HTTP 200 with an errors array when the query is malformed or a filter field name is wrong â€” not a non-200 status. Skipping this check causes silent data loss.
...
Active users is the only SSR-sourced metric on the dashboard â€” it comes from a D1 query counting distinct users in the audit log over the last 24 hours. This is intentional: D1 can be queried server-side at page load for near-instant first paint without waiting for the client-side analytics fetch.
...
### Bento Grid System
2-column grid is the primary layout primitive â€” a simple equal-width grid per row. Each row is independent so column counts can differ, responsive collapse is per-row, and no item unexpectedly spans across sections.
...
- [ ] Setup banner appears with amber styling and dismiss Ã— button
- [ ] All CF-dependent values show `â€”` instead of `0`
...
1. **KV operation counts** â€” Cloudflare does not expose KV reads/writes via any REST or GraphQL API. These entries have been removed rather than shown as permanently N/A.

2. **Supabase Auth API limitations** â€” We fetch `per_page=1000` to do localized counts for provider breakdown, MAU, and recent signups. For projects with >1000 users, this would need pagination logic or dedicated Analytics API integration.

3. **D1 Analytics lag** â€” Cloudflare's Analytics Engine ingests D1 data with ~2-minute lag. Quota bars reflect recent but not real-time write counts.

4. **Workers script name matching** â€” The analytics API returns all scripts on the account. The provider filters client-side to known script names. If a worker is renamed in the Cloudflare dashboard, update the script list in the provider configuration.

5. **Supabase Prometheus endpoint** â€” The privileged metrics endpoint is a non-public API. It provides 20+ crucial infrastructure metrics, but since it's in early beta/internal use, Supabase could change or restrict it without notice. String parsing logic must fail safely.
{% endraw %}
