...
| **E-7** | **Admin and Manager cannot send any email.** `send.ts` reads `email_sender_identities` from D1 and, finding nothing, falls through to the unmapped-alias branch that requires `isOwnerOrVendor` — so every send by a canonical Admin or Manager returns 403. `senders.ts` **does** fall back to `DEFAULT_SENDER_IDENTITIES`, so the composer's dropdown offers addresses the send path will reject. | `src/pages/api/emails/send.ts:133-165`, `src/pages/api/emails/senders.ts:94` | 🔴 **P1** | Found 2026-09-11 re-auditing `5781fb8`. The setting has **never** been written (`SELECT COUNT(*) … = 0`); no migration seeds it and `senders.ts` only persists on create/update/delete. Live impact: **2 of 6 active accounts blocked** (stored `super_admin` and `admin`); owner and dev are unaffected, which is why acceptance missed it. Shipped green because `test/api-email-senders.test.ts` covers the route that *has* the fallback, never the route that *enforces*. **Fix:** give `send.ts` the same `DEFAULT_SENDER_IDENTITIES` fallback (one import, one line) so the two routes agree by construction; seeding the D1 row alone unblocks today but leaves the next fresh environment broken. Add a regression test asserting a Manager can send with the setting absent. See [`features/EMAIL-PORTAL.md`](features/EMAIL-PORTAL.md) §0. |
| E-8 | **The email branding settings are written but read by nothing.** `email_logo_url`, `email_brand_color`, `email_footer_address`, `email_support_phone` and `email_reply_to` are persisted by `engine.ts`'s `save_settings` and read back into the settings form, but `send.ts` reads only `custom_email_max_recipients` and `email_sender_identities`, and the queue payload does not carry them. | `src/pages/api/emails/engine.ts`, `src/pages/api/emails/send.ts` | 🟡 | Found 2026-09-11. The panel is configured-but-not-wired: no customer has ever seen these values. Note the shipped default for `email_support_phone` is `+52 (449) 123-4567`, a **placeholder** — harmless only because nothing reads it, but it must not be wired up as-is (RULE #0.5). Either wire the values into the send payload/templates, or mark the panel as forward-looking in the UI so operators are not misled into thinking it changes outgoing mail. **Partly closed 2026-09-16:** the placeholder phone was pre-filled as the input's VALUE, not its placeholder attribute, so opening the panel and pressing Save persisted an invented number as though an operator had typed it — that fallback is removed, and the panel now carries a visible notice that the branding values are saved but not yet applied to outgoing email. **Still open:** wiring the values into the send payload and templates. That crosses into `cf-email-consumer`, which renders the templates and is a separate repository, so it is a cross-repo change rather than an edit here. |
| E-9 | `brevo_api_key` is a dead D1 read on six routes | `src/pages/api/emails/{engine,senders,quota,dns-check}.ts` | 🔵 | Found 2026-09-11. Six call sites do `getSetting('brevo_api_key')` as a fallback to `env.BREVO_API_KEY`, but nothing writes it and it is absent from `KNOWN_SETTING_KEYS`. `getSetting` is ungated, so each is a real query that always returns `NULL`. Drop the fallback and read the secret only. (Storing a provider secret in `admin_portal_settings` would also cut against ROADMAP chunk 12, which plans to move credentials *out* of that table — worth not starting.) |
| E-10 | The recipient-cap default disagrees between the enforcer and the UI | `src/pages/api/emails/send.ts:109`, `src/pages/api/emails/engine.ts:62` | 🔵 | Found 2026-09-11. `send.ts` falls back to **10**, `engine.ts` to **100**. No live impact while the D1 row exists (both read `10`), but delete that row and the settings panel advertises a cap ten times what is enforced. Single-source the default. |
...
| C-19 | **`src/lib/auth/pipeline.ts` has outgrown its own exception.** It is **620** lines (re-counted 2026-09-02; 721 on 2026-08-23, 608 on 2026-08-13) against the 600-line `max-lines` warning granted to it in `eslint.config.js`. | `src/lib/auth/pipeline.ts`, `eslint.config.js` | ✅ RESOLVED 2026-09-02 | **Resolved 2026-09-02 (viability program chunk 10):** decomposed into `src/lib/auth/stages/` behind a 77-line `pipeline.ts`; the exception is deleted from `eslint.config.js`; `scripts/ratchet.py` A14 holds the files over 600 lines at 21 so no new one can appear. Record: `documentation/program/chunks/2026-09-02-10-auth-pipeline-decomposition.md`. |
...
| C-17 | **Cancel-scheduled-send is documented but does not exist.** `features/EMAIL-PORTAL.md` described `POST /api/emails/cancel` as working behaviour and listed the file twice more in its inventory. There is no `src/pages/api/emails/cancel.ts` and no caller anywhere in `src/`. | `src/pages/api/emails/` | ✅ RESOLVED 2026-09-02 | Spec deleted (viability program chunk 9, roadmap row 9): `EMAIL-PORTAL.md` now states plainly that a scheduled send cannot be cancelled from the portal. If the capability is ever wanted it is a feature request, not a documentation correction. |
...
|---|------|-------|-----|-------|
| D-3 | **A guard gates on a deactivated PLAC page key.** `src/pages/api/users/[id]/session-status.ts` calls `placDenyResponse(session, '/dashboard/users/sessions')`. Migration `migrations/0002_promote_sessions_page.sql` set `is_active = 0` on those `admin_pages` rows when the page moved to `/dashboard/sessions` — confirmed against live D1 on 2026-08-23. The access map is built `WHERE is_active = 1`, so the key is absent from it and `requirePageAccess()` falls through to longest-prefix matching, landing on `/dashboard/users`. | `src/pages/api/users/[id]/session-status.ts`, `src/lib/auth/plac.ts` | ✅ **RESOLVED — verified 2026-09-16** | It does **not** fail open — the parent deny still propagates — but the effective gate is `/dashboard/users`, not the sessions page, so a PLAC grant or deny scoped to sessions has no effect on this endpoint, which returns session telemetry (IP, User-Agent, geolocation, Ray ID). **To close:** decide whether the intended gate is `/dashboard/sessions` (the live page) or `/dashboard/users` (today's effective behaviour), then make the code say so explicitly. Either choice changes who can read session telemetry, so it needs an operator decision and a before/after check of per-role access — not a silent edit. |
| D-4 | **Two page keys for one route family.** `API_PAGE_MAPPING` in `src/lib/auth/routes.ts` maps `/api/sessions` → `/dashboard/sessions`, so the pipeline's default-deny evaluates that page; but all three handlers under `src/pages/api/sessions/` call `placDenyResponse(session, '/dashboard/users')`. Both gates must pass, so the effective requirement is access to **both** pages. | `src/lib/auth/routes.ts`, `src/pages/api/sessions/` | ✅ **RESOLVED — verified 2026-09-16** | Not a hole — the stricter of the two wins — but the mapping and the handlers disagree about which page owns these routes, which makes the access matrix impossible to reason about from either source alone. Resolve to one page key. Related to D-3; fix them together. |

### D-3 / D-4 re-checked 2026-09-16 — both already fixed, and a third gap found

Both entries above described the code as it was; the code had moved and the
entries had not. Verified today: no reference to the deactivated
`/dashboard/users/sessions` key survives anywhere in `src/`, `session-status.ts`
gates on `/dashboard/sessions`, and every handler under `src/pages/api/sessions/`
agrees with `API_PAGE_MAPPING`. Live D1 confirms `/dashboard/sessions` active and
the three `/dashboard/users/sessions*` rows `is_active = 0`.

| # | Gap | Impact |
|---|---|---|
| D-5 | **The sessions sub-permissions were defined but never enforced.** `admin_pages` carries four active rows — `/dashboard/sessions#revoke`, `#unblock`, `#flush`, `#export` — so the access-policy UI offers them as grantable and deniable per user. A grep of `src/` found **no reference to any of them**: every handler gated on the bare page key. | ✅ **RESOLVED 2026-09-16.** This was a deny that did not deny: an owner could refuse an administrator `#revoke`, see the override written and shown in the UI, and that administrator could still revoke sessions — a hash key is not a path descendant, so the page check never covered it. `src/lib/auth/surface-guards.ts` now checks the page key **and** the action key for both surfaces — `denySessions` alongside the `denyCron` it was modelled on (the two `_guard.ts` files were merged there, since a shared permission helper is library code and `src/pages/api/` holds routes; ratchet A18 flagged exactly this): `DELETE active-sessions` → `#revoke`, `DELETE active-revocations` → `#unblock`, `POST flush-sessions` → `#flush`. Reads stay on the page key. `#flush` keeps its existing hardcoded owner check alongside the PLAC one, so the fragment is **deny-only** — it can remove the capability, but a grant alone cannot hand a global destructive action to an administrator. Pinned by `test/sessions-permissions.test.ts`, whose central case is a fragment deny being honoured. `#export` has no server route to gate and is left unmapped. |
...
## Opened and closed by the 2026-09-04 sign-in architecture review

The pure-CFZT single-sign refactor (`198f7bd`) removed the double sign-in: the
`QuickResumeCard` island, its `localStorage` profile memory and the second
"Sign in with Google" click are gone, and Cloudflare Access is now the sole
identity gate. That part was already done and correct. Reviewing the result end
to end surfaced eight items — none of them a regression from that commit, but
the landing page it rewrote had become the single holder of the error and
redirect contract, which is where most of them lived.

All eight were fixed in the same pass. `npm run verify` green; suite 49 files /
708 cases (was 48 / 694). Each row records what was wrong and what closed it.

| # | Item | Where | Severity | Notes |
|---|------|-------|----------|-------|
| C-22 | **Open redirect on the auth landing page.** `returnTo` was read from the query string, `decodeURIComponent`-ed and handed to `Astro.redirect()` with no same-origin check, on both the has-session and the production no-session branch. `https://secure.madagascarhotelags.com/?returnTo=https://example.invalid/` sent a signed-in administrator off-domain from a URL on the trusted admin host. `/` is in `PUBLIC_ROUTES`, so the auth pipeline never inspected the request. | `src/pages/index.astro`, `src/pages/api/auth/dev-login.ts` | ✅ **RESOLVED 2026-09-04** | `safeInternalPath()` in `src/lib/auth/landing.ts` reduces the value to a same-origin path or `null`: decode first (so `%2F%2Fevil` cannot smuggle one through), then require a leading `/`, refuse `//` and `/\` — the two spellings a browser treats alike — and refuse control characters. A malformed escape is a refusal, not a throw. `dev-login.ts` used the same rule in place of its `startsWith('http')` check. The only producer of `returnTo`, the local-dev branch of `src/lib/auth/stages/assertion.ts`, now emits a path rather than the whole URL. Pinned by `test/landing.test.ts`. |
| C-21 | **Four `?error=` codes never reached a screen.** `missing_token`, `expired_token`, `session_revoked` and bootstrap's `access_revoked` were appended to `/cdn-cgi/access/logout` — Cloudflare's endpoint, which does not forward an `error` parameter to the application — so the user landed on Cloudflare's logout page instead of the card written for them. Three codes that did reach `/` (`access_revoked`, `recheck_failed`, `system_error`) had no `ERROR_DESCRIPTIONS` entry and fell through to the generic card, and `account_inactive` had a card but was never emitted. Two of six screens worked. | `src/pages/index.astro`, `src/lib/auth/stages/assertion.ts`, `src/lib/auth/stages/session-stage.ts`, `src/lib/auth/stages/bootstrap.ts` | ✅ **RESOLVED 2026-09-04** | All four now redirect to `/?error=<code>`, which is a page that can read the code; the Cloudflare logout stays where it belongs, behind the card's "Sign In with Another Account" button. **The fix deliberately does not use `returnTo` on the logout endpoint** — Cloudflare's session-management docs describe no such parameter, so building on it would be building on an undocumented default. Seven descriptions added (`access_revoked`, `missing_identity`, `role_unrecognised`, `recheck_failed`, `system_error`, plus `account_inactive` now genuinely emitted), and bootstrap distinguishes a deactivated account from one that was never authorized while keeping the API answer a flat 403. |
| C-20 | **Two pipeline branches did not fork on API vs page.** A request with no `CF-Access-Authenticated-User-Email` redirected to `/` even when the path was `/api/*`, so an API client got an HTML landing page; an untranslatable stored role returned JSON 403 even on a page, so a browser got a raw JSON body. | `src/lib/auth/stages/assertion.ts`, `src/lib/auth/stages/bootstrap.ts` | ✅ **RESOLVED 2026-09-04** | Both fork now: `401 {"error":"Missing identity"}` / `403 {"error":"Account role not recognised"}` for `/api/*`, and `/?error=missing_identity` / `/?error=role_unrecognised` for a page. Both directions are pinned in `test/pipeline-bootstrap.test.ts`. The unrecognised-role branch remains the one refusal in `bootstrap.ts` that emits no login event; the file's header comment claimed otherwise and has been corrected rather than the behaviour changed. |
| C-23 | **`/` and `/dashboard` redirected to each other with no cycle breaker.** `/` is public, so the pipeline skips it; with no session and no `?error=` the page redirected to `/dashboard` to trip the Access challenge, and `/dashboard` with no identity header redirected back to `/`. The pair terminated only because Cloudflare Access answered `/dashboard` before the origin did. | `src/pages/index.astro`, `src/lib/auth/stages/assertion.ts` | ✅ **RESOLVED 2026-09-04** | The no-identity branch sends `/?error=missing_identity` instead of a bare `/`, and `decideLanding()` renders an error card **before** the production bounce, so the cycle cannot close. Never reachable in production while Access covers `/*` and `workers_dev = false`, but it no longer depends on that. |
| C-27 | **The landing page decided dev mode partly from the request hostname.** `devMode = isLocalDev(env.SITE_URL) \|\| Astro.url.hostname === 'localhost' \|\| === '127.0.0.1'`, against a fail-secure rule stated as `SITE_URL` alone precisely so a misconfigured variable could never enable dev mode. Not reachable through the Cloudflare edge, which routes on hostname; the blast radius if it ever were is the Dev User Picker rendering every active administrator's email, name and role. | `src/pages/index.astro`, `src/pages/api/auth/dev-login.ts`, `src/pages/api/content/edge-verify.ts` | ✅ **RESOLVED 2026-09-04** | The two hostname clauses are gone; `SITE_URL` alone decides. The three implementations of the predicate are now one: `dev-login.ts` and `edge-verify.ts` import `isLocalDev` from `src/lib/auth/routes.ts` instead of carrying private copies. |
| C-25 | **The entire `/api/auth/` prefix was exempt from the pipeline** — the prefix, not the two paths under it — so any file added to `src/pages/api/auth/` would be unauthenticated and PLAC-exempt from the moment it existed. `GET /api/auth/logout` also had no CSRF gate, so an `<img src>` on any page could force-logout an administrator. | `src/lib/auth/routes.ts`, `src/pages/api/auth/logout.ts` | ✅ **RESOLVED 2026-09-04** | `/api/auth/logout` and `/api/auth/dev-login` are named individually in `PUBLIC_API_ROUTES`; the prefix is deleted, so a third file in that directory is now default-deny until someone adds it deliberately. `test/api-authz-inventory.test.ts` pins the exact set. The GET logout refuses a `Sec-Fetch-Site` of anything but `none` (bookmark, typed URL) or `same-origin` (the in-app fallback in `Sidebar` and `TopBar`) without touching the session; a browser that omits the header is still served. |
| C-24 | **The landing page had no test.** `src/pages/index.astro` grew from 31 to 278 lines in `198f7bd` and held the session bounce, the production CFZT trip, the error-screen contract and the dev picker; the pipeline beside it had 47 characterization cases and this file had zero. | `src/pages/index.astro`, `test/landing.test.ts` | ✅ **RESOLVED 2026-09-04** | The rules moved to `src/lib/auth/landing.ts` as two pure functions and the page calls them, because `.astro` frontmatter cannot be exercised under the Workers pool. `test/landing.test.ts` adds 13 cases over the redirect precedence and every open-redirect spelling. |
| C-26 | **`preview_urls` was left to a Wrangler default.** `workers_dev = false` is set and commented as the thing that closes the off-Access ingress; version preview URLs are a second `*.workers.dev` surface the same reasoning covers, and the setting was not declared. | `wrangler.toml` | ✅ **RESOLVED 2026-09-04** | `preview_urls = false` declared next to `workers_dev`. It was already safe — Cloudflare defaults it to the `workers_dev` value on Wrangler v4.44.0+ and ran a one-time disable for Workers with `workers.dev` off, and this repo pins `wrangler ^4.128.0` — but that default has changed twice, and the single-ingress guarantee should not rest on it. |

### What this pass did not change

- **The JWT verifier.** `src/lib/auth/cloudflare-access.ts` pins `alg: RS256`, requires `kid`, re-fetches the JWKS once on rotation, and checks `exp`, `iat` skew, `iss` and `aud`. Reviewed, no changes.
- **`resolveLoginMethod` no longer invents a provider (C-28, opened and closed 2026-09-05).** Any IdP type that was not GitHub or one-time-PIN was recorded as a **Google** sign-in in `admin_login_logs.login_method` and named as such in the sign-in alert email; `login_method` carries no CHECK constraint, so nothing stopped it. The display half invented in the opposite direction: `loginMethodLabel` rendered every unmatched value — `null` included — as **OTP**. Two different fabricated answers to "how did this person authenticate", on a security forensics surface, which is RULE #0.5. `resolveLoginMethod` now returns `'unknown'` and matches Google explicitly (`google`, `google-apps`); the label and icon helpers report Unknown rather than OTP. The raw claim was never lost — it is stored and displayed as `cf_identity_provider` — so only the coarse bucket changes. Pinned by `test/login-method.test.ts`. This was initially deferred to C-10 as forensics accuracy rather than a sign-in defect; that was the wrong call once the missing constraint and the second fabrication surfaced.
- **The revocation path.** The redirects that used to go straight to Cloudflare's logout endpoint now show a card first. Enforcement is unchanged: the KV revocation flag still blocks re-bootstrap in `src/lib/auth/stages/bootstrap.ts`, the app session is still destroyed, and the Layer 3 Cloudflare session revoke still happens at revocation time in `src/lib/auth/plac.ts`. The logout hop was convenience, not the control.
