...
| C-8 | **No prod/staging separation.** Gap G14. | `wrangler.toml` | 🟡 | Needs new Cloudflare resources; `RULESAd.md` §12 makes inventing binding UUIDs a production-outage risk, so this is an operator decision. |
...
| C-15 | **`knip` is wired but not clean, and `knip.json` is misconfigured for Astro.** `npm run knip` now exists (added 2026-08-13; knip 5.88.1 resolves from the monorepo root). It exits 1. | `package.json`, `knip.json` | 🔵 | Current output: **34 "unused" files** — almost certainly false positives, because `knip.json` declares no `entry` patterns and so cannot see Astro's file-based routes (`src/pages/**`) or the Worker entrypoint (`src/workers/cf-entry.ts`); **5 unused exported types**; and **4 "duplicate exports"** which are the intentional deprecated role aliases (`isAdmin`/`isManagerOrAbove`, `isSuperAdmin`/`isAdminOrAbove`, `isDev`/`isVendorSupport`, `isOwnerOrDev`/`isOwnerOrVendor` — see `architecture/plac-and-audit.md` §1.4). **To close:** add `entry` patterns to `knip.json` first, re-run, and only then judge the real findings. `RULESAd.md` previously claimed the sweep "must be 100% clean"; it has never been clean, and that claim has been corrected to match. |
| C-19 | **`src/lib/auth/pipeline.ts` has outgrown its own exception.** It is **721** lines against the 600-line `max-lines` warning granted to it in `eslint.config.js`, so `npm run lint` reports 1 `max-lines` warning (plus 474 `no-explicit-any`). Re-counted 2026-08-23; it was recorded as 608 and the exception's own comment still says 517. | `src/lib/auth/pipeline.ts`, `eslint.config.js` | 🔵 | The exception's comment still describes the file as "517 lines" and argues the 3.4% overage does not justify splitting security-critical control flow. At 721 — a 20% overage, and 40% past the figure the comment cites — that argument no longer holds and the number has now drifted twice. Decide: raise the ceiling with a fresh rationale, or extract a cohesive piece (the shadow-deny/`API_DENY_MODE` branch is the most separable). Do not simply re-raise the number without updating the comment — that is how it drifted. |
