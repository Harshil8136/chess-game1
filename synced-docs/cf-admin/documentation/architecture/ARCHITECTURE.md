...
> **Live module set (verified 2026-06-06 against `src/pages/dashboard/`):** the
> nine dashboard modules below. `customers`, `pets`, `analytics`, and `reports`
> appear as audit-log enum *modules* (`src/lib/audit.ts`) but are **not** dashboard
> routes; they remain in the Scale-Up Vault (§8) until needed.
...
│   ├── content/index.astro (+ gallery, services, reviews, faqs, stats, media sub-pages)
│   ├── chatbot/index.astro (+ analytics, config, KB, prompts, models, conversations sub-tabs)
│   ├── control-plane/index.astro (+ sentry, cloudflare, posthog, supabase sub-pages)
│   ├── logs/index.astro
...
→ See [DESIGN-SYSTEM.md](../reference/DESIGN-SYSTEM.md)
...
→ See [CODING-STANDARDS.md](../reference/coding-standards.md) for DAL patterns and code conventions
...
→ See [CHATBOT.md](../features/CHATBOT.md)
