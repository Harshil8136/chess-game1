...
| `documentation/program/` | The long-term viability program (started 2026-09-02): the evergreen roadmap and debt registry, a chunk-record template, dated chunk records under `program/chunks/`, and decision records under `program/adr/`. Chunk records follow §3's dated naming and move `draft` → `active` → `historical` as they ship (see `program/CHUNK-TEMPLATE.md`). Excluded from the public docs mirror by decision ADR-0001 |
