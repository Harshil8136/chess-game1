...
- **Run `npm run verify` before every push** — it is the only gate that runs before the code is live, because the deploy does not wait for CI. Since 2026-09-15 it includes the quality ratchet (`scripts/ratchet.py`, baseline in `.ratchet.json`): nine debt counts that may only fall — a count that rises fails `verify` and CI until `python3 scripts/ratchet.py --update --reason "why"` is committed with the change; a count that falls is locked in with `--update`.
