...
- **Traffic is low — on the order of ~50 requests/day** (per the Worker observability
  ...
  volumetric/DDoS traffic *before* it reaches the Worker. Origin-side rate limiting is
...
*No application behavior was changed beyond the security fixes listed in §3.*
