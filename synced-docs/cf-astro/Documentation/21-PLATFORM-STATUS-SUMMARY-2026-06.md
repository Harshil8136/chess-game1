{% raw %}
# 21 — Platform Status & Security Summary (2026-06-13)

**Covers both repositories:** `cf-admin-madagascar` (admin portal) and `cf-astro` (public booking site).
...
repos:
  cf-admin-madagascar:
    { role: admin portal, runtime: Cloudflare Worker (Astro SSR), grade: 'A- (91/100)' }
  cf-astro:
    {
      role: public booking + marketing site,
      runtime: Cloudflare Pages/Worker (Astro SSR),
      grade: 'A- (90/100)',
    }
...
do_not_regress: # invariants confirmed safe in this review — keep them
  - parameterized SQL only (Drizzle + D1 binds); never string-build queries
  ...
  - public endpoints stay origin-guarded + rate-limited; webhooks stay signature-verified
...
- **`cf-astro`:** closed 3 Medium + low items — shared-state rate limiting, personal-data
  redaction before audit writes, credential rotations, identity-form (Turnstile) hardening,
  preventive structured-data escaping, schema-based validation.
- **`cf-admin`:** closed 1 High + 4 Medium + low/info items — dev-login production guard,
  ...
  fail-closed role rechecks, JWT issuer pinning.
...
change** and likely no cost change. The _first_ real constraints, far above any near-term
...
| **Build & type quality** | `cf-astro`: `tsc --noEmit` ✅ (full `astro build` runs in CI, which has the required Cloudflare token). `cf-admin`: `astro build` ✅ and `astro check` ✅ **0 errors / 0 warnings / 0 hints** (299 files).   |
| **Category comparison**  | Versus typical Astro/Cloudflare marketing sites and small-business admin panels (no rate limiting, no webhook verification, no RLS, inline secrets), this platform sits in the **top decile**.               |
...
- **Supabase plan is the only real lever.** Pro (~$25/mo) buys daily backups + no
  ...
  idle and has no point-in-time recovery. **Recommendation:** keep Pro _only if_ backup/uptime
...
- **Measured this pass (2026-06-13):** `npm ci`, `tsc --noEmit` (both),
  `astro build`/`astro check` (cf-admin), `npm audit` (both).
...
_This summary is a point-in-time status (2026-06-13). No application behavior was changed by
producing it. It is published to both repositories and mirrored to the public documentation repo._
